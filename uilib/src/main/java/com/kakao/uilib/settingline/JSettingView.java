package com.kakao.uilib.settingline;
 ...
 * 时间：2016/12/21 10:32
 *
 * github:https://github.com/leonHua/LSettingView
 * 修改：jinyb
 *
 属性	    说明	    类型
 ...
 另外添加了 isShowTopLine diverColor  TopLeft TopRight BottomLeft BottomRight
 方便对线条的灵活的控制

 *
 *
 */
public class JSettingView extends RelativeLayout {
    /*左侧显示文本*/
    ...
    public JSettingView(Context context) {
        this(context, null);
    }

    public JSettingView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public JSettingView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void getCustomStyle(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            } else if (attr == R.styleable.LSettingView_diverColor) {
                int diverColor = a.getColor(attr,Color.parseColor("#99999999"));
                mUnderLine.setBackgroundColor(diverColor);
                mTopLine.setBackgroundColor(diverColor);
            } else if (attr == R.styleable.LSettingView_isShowUnderLine) {
                //默认显示分割线
                if (!a.getBoolean(attr, true)) {
                    mUnderLine.setVisibility(View.GONE);
                }
            } else if (attr == R.styleable.LSettingView_isShowTopLine) {
                //默认显示分割线
                if (!a.getBoolean(attr, true)) {
                    mTopLine.setVisibility(View.GONE);
                }
            } else if (attr == R.styleable.LSettingView_BottomLineLeft) {
                int bottomLeft = (int) a.getDimension(attr, 0);
                RelativeLayout.LayoutParams layoutParams = (LayoutParams) mUnderLine.getLayoutParams();
                layoutParams.leftMargin = bottomLeft;
                mUnderLine.setLayoutParams(layoutParams);

                Log.e("left margin",bottomLeft+"");

            } else if (attr == R.styleable.LSettingView_BottomLineRight) {
                int bottomRight = (int) a.getDimension(attr, 0);
                RelativeLayout.LayoutParams layoutParams = (LayoutParams) mUnderLine.getLayoutParams();
                layoutParams.rightMargin = bottomRight;
                mUnderLine.setLayoutParams(layoutParams);

            } else if (attr == R.styleable.LSettingView_TopLineLeft) {
                int left = (int) a.getDimension(attr, 0);
                RelativeLayout.LayoutParams layoutParams = (LayoutParams) mTopLine.getLayoutParams();
                layoutParams.leftMargin = left;
                mTopLine.setLayoutParams(layoutParams);
            } else if (attr == R.styleable.LSettingView_TopLineRight) {
                int right = (int) a.getDimension(attr, 0);
                RelativeLayout.LayoutParams layoutParams = (LayoutParams) mTopLine.getLayoutParams();
    ...
    private void initView(Context context) {
        ...
        mTopLine = (View) mView.findViewById(R.id.topline);
